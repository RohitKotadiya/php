<?php
    require_once "registration_PostData.php";
?>
<!DOCTYPE html>
<html lang="en">
    <head>
    ...
    <body>
        <form action="registrationForm.php" method="POST" enctype="multipart/form-data">
            <fieldset id="account-info">
                ...
                <input type="text" name="account[firstName]" required value="<?= getFieldValue('account', 'firstName') ?>">
                <?php if(in_array("fName" , $errList))   :?>
                    <span>Invalid first Name</span>
                <?php endif; ?><br>
                <label>Last Name</label>
                <input type="text" name="account[lastName]" required value=<?= getFieldValue("account", "lastName") ?>>
                <?php if(in_array("lName" , $errList))   :?>
                    <span>Invalid last Name</span>
                <?php endif; ?><br>
                <label>Date Of Birth</label>
                <input type="date" name="account[dateOfBirth]" value=<?= getFieldValue('account', 'dateOfBirth')?> > <br>
                
                <label>Phone Number</label>
                <input type="number" name="account[phoneNumber]" required value=<?= getFieldValue('account', 'phoneNumber') ?>><br>

                <label>Email Id</label>
                <input type="text" name="account[emailAddress]" required value=<?= getFieldValue('account', 'emailAddress') ?> ><br>

                <label>Password</label>
                <input type="password" name="account[password]" value = <?= getFieldValue('account', 'password')?>><br>
            ...
            <fieldset id="address-info">
                ...
                <input type="text" name="address[addressLineOne]" value= <?= getFieldValue('address', 'addressLineOne') ?> ><br>

                <label>Address Line 2</label>
                <input type="text" name="address[addressLineTwo]" value= <?= getFieldValue('address', 'addressLineTwo') ?> ><br>
